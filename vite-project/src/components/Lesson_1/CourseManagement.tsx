...
const CourseManagement: React.FC = () => {
  ...
  const handleValidate = () => {
    ...
    } else if(newCourse.students === "" || parseInt(newCourse.students as string) <= 0) {
  ...
  const handleAddCourse = () => {
    ...
    setNewCourse({ name: "", description: "", duration: "", students: "".toString() });
            ...
            <tr key={course.id} className="course-row">
              {editingCourse?.id === course.id ? (
                <>
                  ...
                  <td><input type="number" className="form-input" value={editingCourse.students} onChange={(e) => setEditingCourse({...editingCourse, students: parseInt(e.target.value) || "".toString()})} /></td>
                  <td>
          ...
          <div className="modal-content">
            ...
            <div className="modal-body">
              ...
              <div>
                ...
                />
                {errors.duration && <p style={{ color: '#ef4444', fontSize: '12px', marginTop: '4px' }}>{errors.duration}</p>}
              </div>
              
              <div>
                <input
                  className={`form-input ${errors.students ? 'input-error' : ''}`}      
                  type="number"
                  min="0"
                  placeholder="Số học viên"
                  value={newCourse.students}
                  onChange={(e) => setNewCourse({...newCourse, students: parseInt(e.target.value) || "".toString()})}
                />
                {errors.students && <p style={{ color: '#ef4444', fontSize: '12px', marginTop: '4px' }}>{errors.students}</p>}
              </div>
